...
export class MarketplaceStore {
  ...
  constructor() {
    makeAutoObservable(
      this,
      { shownDevices: computed, cartCount: computed, cartTotal: computed },
      { autoBind: true },
    );
  }
  ...
  get shownDevices() {
    ...
    const matches = this.devices.filter(
      (device) =>
  ...
  async initialize() {
    try {
      ...
      this.favorites = JSON.parse(
        localStorage.getItem("atelier-favorites") || "[]",
      );
    } catch {}
    try {
  ...
  setSelected(value: Device | null) {
    this.selected = value;
  }
  setCartOpen(value: boolean) {
    this.cartOpen = value;
  }
  setAuthOpen(value: boolean) {
    this.authOpen = value;
  }

  private basketToCart(basket: Basket): CartItem[] {
    return (basket.basket_devices || [])
