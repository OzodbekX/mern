...
export default function AuthModal({close}:{close:()=>void}){
 ...
 async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();setBusy(true);setMessage("");const data=new FormData(e.currentTarget);const payload={email:String(data.get("email")),password:String(data.get("password"))};try{const result=mode==="login"?await api.users.login(payload):await api.users.register(payload);localStorage.setItem("atelier-token",result.token);setMessage("Welcome to Atelier Market.");setTimeout(close,900)}catch(error){setMessage(error instanceof Error?error.message:"Something went wrong.")}finally{setBusy(false)}}
