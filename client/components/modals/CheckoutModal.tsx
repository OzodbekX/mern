...
export default function CheckoutModal({ cart, total, close, onPaid }: Props) {
  ...
  const [cards, setCards] = useState<SavedCard[]>([]),
    ...
    [pending, setPending] = useState<PendingOrder | null>(null),
    [saveAfterPayment, setSaveAfterPayment] = useState(false),
    [newCardMeta, setNewCardMeta] = useState<CardPayload | null>(null);
  const token =
    typeof window !== "undefined"
      ? localStorage.getItem("atelier-token")
      : null;
  ...
  const paySaved = async (cardId: number) => {
    ...
    setMessage("");
    try {
      await api.orders.purchaseWithSavedCard(
        { items, amount: total, cardId },
        token,
      );
      await onPaid();
  ...
  const verify = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await api.orders.verify(
        { transactionId: pending.transactionId, verificationCode: code },
        token,
      );
      if (saveAfterPayment && newCardMeta) {
        const saved = await api.cards.create(newCardMeta, token);
  ...
  return (
    <div
      className="fixed inset-0 z-[80] grid place-items-center bg-[#181814]/70 p-4 backdrop-blur-sm"
      onMouseDown={(event) => event.target === event.currentTarget && close()}
    >
      <div className="relative max-h-[92vh] w-full max-w-2xl overflow-auto rounded-[28px] bg-[#fbfaf6] p-6 shadow-2xl md:p-10">
        <button
          className="absolute top-5 right-5 grid h-10 w-10 place-items-center rounded-full bg-[#eee8db]"
          onClick={close}
        >
          <Icon name="close" />
        </button>
        ...
        {pending ? (
          ...
          <>
            <div className="mb-6 grid grid-cols-2 rounded-xl bg-[#eee8db] p-1">
              <button
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                {cards.length ? (
                  cards.map((card) => (
                    <div
                      key={card.id}
                      className="flex items-center justify-between rounded-2xl border border-[#d8d2c5] p-4"
                    >
                      <div>
                        <b className="font-serif text-lg">
                          {card.brand} •••• {card.last4}
                        </b>
                        <p className="mt-1 text-[10px] text-[#716f67]">
                          {card.holderName} ·{" "}
              ...
              <NewCardForm
                busy={busy}
                save={saveAfterPayment}
                setSave={setSaveAfterPayment}
                onSubmit={submitNew}
              />
            )}{" "}
            {message && (
              <p className="mt-4 rounded-xl bg-[#f4e4de] p-3 text-center text-xs text-[#a34328]">
                {message}
              </p>
            )}
          </>
...
function Field({
  ...
  return (
    <label className="text-[9px] uppercase tracking-[.12em]">
      {label}
      <input
        {...props}
        required
        className="mt-2 w-full rounded-xl border border-[#d8d2c5] bg-white px-4 py-3 text-xs normal-case tracking-normal outline-none focus:border-[#b95736]"
      />
    </label>
  );
}
  ...
  return (
    <form onSubmit={onSubmit} className="py-4 text-center">
      ...
      <input
        ...
        inputMode="numeric"
        maxLength={6}
        className="mx-auto mt-6 block w-48 rounded-xl border border-[#d8d2c5] bg-white px-4 py-3 text-center text-xl tracking-[.35em] outline-none focus:border-[#b95736]"
        placeholder="000000"
      />
