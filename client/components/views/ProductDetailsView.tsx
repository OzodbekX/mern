...
export default function ProductDetailsView({ id }: { id: string }) {
  ...
  const addToCart = async () => {
    ...
    if (!token) {
      setBasketMessage("Please sign in from the marketplace before adding this item.");
      return;
    }
    try {
      ...
      setBasketMessage(error instanceof Error ? error.message : "Could not update your basket");
    }
  };
